...
export const createFeedAutoPost = async (
  ...
  try {
    ...
    if (type === "MATCH_CONFIRMED") {
      ...
      if (existing) {
        console.log("MATCH_CONFIRMED post already exists for this match");
    ...
    if (type === "REL_STATUS_UPDATE" && payload.phase) {
      ...
      if (existing) {
        console.log(`REL_STATUS_UPDATE post already exists for phase ${payload.phase}`);
        return { data: existing, error: null };
      }
    }
    ...
    console.log(`Feed auto-post created: ${type} for match ${matchId}`);
