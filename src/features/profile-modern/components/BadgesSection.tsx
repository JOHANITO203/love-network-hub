    ...
    <div className="relative">
      <motion.div
        ...
        className={`
          ...
          border ${isLocked ? 'border-white/10' : 'border-white/40'}
          shadow-lg
        `}
      >
        <span className={`text-3xl ${isLocked ? 'opacity-40 grayscale' : ''}`}>
          {badge.icon}
        </span>
        ...
        {!isLocked && (
          <div
            className={`absolute -top-1 -right-1 w-5 h-5 rounded-full ${rarityConfig.color} border-2 border-[#0b0f1a]`}
          />
        )}
      </motion.div>
      ...
      <AnimatePresence>
        {isHovered && (
          ...
          >
            <div className="glass-panel text-white rounded-xl p-3 shadow-xl border border-white/10">
              <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-[#111625] border-r border-b border-white/10 rotate-45" />
